...
var fileUtil = require('../bi_log_tool/file_util.js');
var logger = require('../manager/log4_manager.js');
var db = require('../bi_log_tool/db_util.js');
var writeDBConfig = require('../../server_config/mysql.json');

var isEnd = true;/*是否一轮操作结束*/
var buffer = new Buffer(10485760/5);
var rootPath = path.resolve('../logs/');
var runTimeFullPath = path.join(rootPath, define.RUNTIME_GM_FILENAME);
db.setToolType(2);
...
var execute = function(callback) {
    ...
    async.waterfall([
        function (callback) {
            /*获取上次写到哪里*/
            fileUtil.readLogRunTime(rootPath, define.GM_LOGPRE, runTimeFullPath, function(err, data){
                if(err){
                    callback(err);
                }
                else{
        ...
        function (callback) {
            ...
            fileUtil.getGameLog(rootPath, logRunTime.fileNameEnd, buffer, buffer.length, logRunTime.offset,
                ...
                if (err == define.FU_4) {
                    /*检查是否有新增文件*/
                    var newFileName = fileUtil.checkNewAddFileName(rootPath, define.GM_LOGPRE, logRunTime.fileNameEnd);
                    if (!newFileName) {
                        callback(define.W_3);
                    }
                    else{
        ...
        function (logContentArr, callback) {
            if(logContentArr.length < 1){
                ...
                if(newSurplusStr != logRunTime.surplusStr){
                    /*更新logRunTime*/
